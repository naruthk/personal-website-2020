  ...
  return (
    <Layout pageTitle="Blog">
      ...
      <Container>
        ...
        <p
          css={css`
            ${tw`md:mb-6 text-sm sm:text-base text-gray-600`}
          `}
        >
          I mostly write on topics related to web development and best practices
          with content about trending news and stories thrown into the mix
          occasionally.
        </p>
      </Container>
      <BlogComponentWrapper>
        <FeaturedPostsWrapper>
          {posts.slice(0, NUMBER_OF_FEATURED_POSTS).map(post => {
        ...
        {posts.length > NUMBER_OF_FEATURED_POSTS && (
          <>
            ...
            <RemainingPostsWrapper>
              {posts.slice(NUMBER_OF_FEATURED_POSTS).map(post => {
...
BlogsListingPage.propTypes = {
  data: PropTypes.shape({
    allContentfulBlogPosts: PropTypes.shape({
      edges: PropTypes.arrayOf(BlogItem),
    }),
  }).isRequired,
};
